...
class Adaptive_Module(nn.Module):
    def __init__(self, in_ch=3, nf=32, tm_pts_num=8, gamma_range=[1.,4.]):
        ...
        self.head1 = BaseConv(16, nf, ksize=5, stride=5)
        ...
        self.head2 = BaseConv(16, nf, ksize=5, stride=5)
    ...
    def forward(self, img, rgb):
        # img_down = F.interpolate(img, (256, 256), mode='bilinear', align_corners=True)
        # h, w = img.shape[-2:]
        h, w = rgb.shape[-2:]
        # img = F.interpolate(img, (256, 256), mode='bilinear', align_corners=True)
        # fea = self.head1(img_down)
        fea = self.head1(img)
        # print("shape2=====",fea.shape) 
        # shape===== torch.Size([4, 16, 128, 128])  batchsize channel w h
        # assert 0
        ...
        out_gamma = self.apply_gamma(rgb, para_gamma)

        # fea_local = self.head2(img_down)
        fea_local = self.head2(img)
        ...
        out_local = self.apply_local(rgb, param_local)

        out = self.out_layer((out_local + out_gamma)/2)
        # out = self.out_layer(out_local )
