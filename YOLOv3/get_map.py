...
if __name__ == "__main__":
    ...
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    # map_out_path    = 'map_out/3'
    # map_out_path    = 'map_out/nolocal'
    map_out_path    = 'map_out/noglobal/1'
    # map_out_path    = 'map_out/RID'
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test.txt")).read().strip().split()
    # image_ids = open(os.path.join(VOCdevkit_path, "RID/ImageSets/Main/val_false.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            # rgb_path  = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/"+image_id+".jpg")
            # rgb       = cv2.imread(rgb_path, cv2.IMREAD_UNCHANGED)
            # rgb         = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)

            # raw_path=os.path.join(VOCdevkit_path, "VOC2007/RAWImages/"+image_id+".png")
            # raw = cv2.imread(raw_path, cv2.IMREAD_UNCHANGED)
            # raw           = cv2.cvtColor(raw, cv2.COLOR_BGR2RGB)


            # rgb_path  = os.path.join(VOCdevkit_path, "RID/JPEGImages/"+image_id+".jpg")
            # rgb       = cv2.imread(rgb_path, cv2.IMREAD_UNCHANGED)
            # rgb         = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)

            # raw_path=os.path.join(VOCdevkit_path, "RID/RAWImages/"+image_id+".png")
            # raw = cv2.imread(raw_path, cv2.IMREAD_UNCHANGED)
            # raw           = cv2.cvtColor(raw, cv2.COLOR_BGR2RGB)
            ...
            # raw_path=os.path.join(VOCdevkit_path, "AODRaw/RAWImages/"+image_id+".npy")
            # raw  = np.load(raw_path)
            # raw  = np.transpose(raw,(1,2,0))  
            count+=1
            bpp = yolo.get_map_txt(image_id, rgb, raw, class_names, map_out_path)
            total_bpp+=bpp
        total_bpp/=count
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                ...
                # root = ET.parse(os.path.join(VOCdevkit_path, "RID/Annotations/"+image_id+".xml")).getroot()
    ...
    if map_mode == 0 :
        print("Get map.")
        map,map50,map75,_,_,_,_,_,_,_,_,_=get_coco_map(class_names = class_names, path = map_out_path)
        print("map=%.6f"%(map))
        print("map50=%.6f"%(map50))
        print("map75=%.6f"%(map75))
        print("Get map done.")
    
    print("bpp=%.6f"%(total_bpp))
