...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        images_raw, images_rgb, targets = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                images_raw  = images_raw.cuda(local_rank)
        ...
        if not fp16:
            ...
            #   前向传播
            #----------------------#
            outputs  ,bpploss      = model_train(images_raw,images_rgb)
            lambda_bpp=1  ###bpploss的weights
            bpploss*=lambda_bpp
            ...
            loss_value = loss_value_all+bpploss
