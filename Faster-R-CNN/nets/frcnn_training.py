...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, imgs_rgb, bboxes, labels, scale):
        n           = imgs.shape[0]
        ...
        base_feature ,bpploss= self.model_train(imgs, imgs_rgb, mode = 'extractor')

        # -------------------------------------------------- #
        #   利用rpn网络获得调整参数、得分、建议框、先验框
        # -------------------------------------------------- #
        rpn_locs, rpn_scores, rois, roi_indices, anchor = self.model_train(x = [base_feature, img_size],rgb=imgs_rgb, scale = scale, mode = 'rpn')
        ...
        roi_cls_locs, roi_scores = self.model_train([base_feature, sample_rois, sample_indexes, img_size],imgs_rgb, mode = 'head')
        for i in range(n):
            # ------------------------------------------------------ #
        ...
        losses = losses +[bpploss]+ [sum(losses)]
        return losses

    def train_step(self, imgs,imgs_rgb, bboxes, labels, scale, fp16=False, scaler=None):
        self.optimizer.zero_grad()
        if not fp16:
            losses = self.forward(imgs, imgs_rgb, bboxes, labels, scale)
            # lambda=1
            loss=losses[-1]+losses[-2]*0.5
            loss.backward()
            self.optimizer.step()
        else:
            assert 0
