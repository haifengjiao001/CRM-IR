...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------------------------#
    # dir_origin_path = "../../VOCdevkit/VOC2007/TEST_IMAGE/LOD/JPG_LOD_Test"
    # dir_save_path   = "img_out/lod_test"
    dir_origin_path = "img/rgb"
    ...
    if mode == "predict":
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                rgb_path  = os.path.join(dir_origin_path, img_name)

                # 替换路径中的 'rgb' 为 'raw'
                # new_dir_path = dir_origin_path.replace('JPG_LOD_Test', 'RAW_LOD_Test')
                new_dir_path = dir_origin_path.replace('rgb', 'raw')
                # 替换文件扩展名 '.jpg' 为 '.png'
                new_file_name = img_name .replace('jpg', 'png')
                # 组合新的路径
                raw_path = os.path.join(new_dir_path, new_file_name)
                ...
                draw_image= Image.open(rgb_path)
                r_image     = frcnn.detect_image(rgb,raw,draw_image)
