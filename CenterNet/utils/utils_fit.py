...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, backbone, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        batch_images, batch_raws, batch_hms, batch_whs, batch_regs, batch_reg_masks = batch
        ...
        if not fp16:
            if backbone=="resnet50":
                hm, wh, offset,bpp_loss  = model_train(batch_images,batch_raws)
                c_loss          = focal_loss(hm, batch_hms)
                wh_loss         = 0.1 * reg_l1_loss(wh, batch_whs, batch_reg_masks)
                off_loss        = reg_l1_loss(offset, batch_regs, batch_reg_masks)
                bpp_lambda=1
                loss            = c_loss + wh_loss + off_loss + bpp_loss*bpp_lambda

                total_loss      += loss.item()
                total_c_loss    += c_loss.item()
                total_r_loss    += wh_loss.item() + off_loss.item()
                # total_bpp_loss  += bpp_loss.item()
            else:
                assert 0
                outputs         = model_train(batch_images,batch_raws)
                loss            = 0
                c_loss_all      = 0
                r_loss_all      = 0
                bpp_loss_all    = 0
            ...
            with autocast():
                if backbone=="resnet50":
                    ...
                    loss            = c_loss + wh_loss + off_loss+ bpp_loss
        ...
        if local_rank == 0:
            pbar.set_postfix(**{
                                ...
                                # 'train_bpp_loss'  : total_bpp_loss / (iteration + 1),
                                'lr'              : get_lr(optimizer)})
            pbar.update(1)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            if backbone=="resnet50":
                hm, wh, offset,bpploss  = model_train(batch_images,batch_raws)
                c_loss          = focal_loss(hm, batch_hms)
                wh_loss         = 0.1 * reg_l1_loss(wh, batch_whs, batch_reg_masks)
                off_loss        = reg_l1_loss(offset, batch_regs, batch_reg_masks)

                loss            = c_loss + wh_loss + off_loss + bpploss*bpp_lambda
