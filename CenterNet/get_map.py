...
if __name__ == "__main__":
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "RID/ImageSets/Main/val.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "RID/JPEGImages/"+image_id+".jpg")
            # image_path  = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/"+image_id+".jpg")
            image       = Image.open(image_path)

            image_raw_path=os.path.join(VOCdevkit_path, "RID/RAWImages/"+image_id+".png")
            # image_raw_path=os.path.join(VOCdevkit_path, "VOC2007/RAWImages/"+image_id+".png")
            raw = cv2.imread(image_raw_path, cv2.IMREAD_UNCHANGED)

            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))
            count+=1
            bpp=centernet.get_map_txt(image_id, image,raw, class_names, map_out_path)
            total_bpp+=bpp
        total_bpp/=count
